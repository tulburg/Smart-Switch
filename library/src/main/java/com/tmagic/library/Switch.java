...
public class Switch extends LinearLayout{
        ...
        protected void onFinishInflate() {
            ...
            for(int i=getChildCount() -1;i>=0;i--) {
                if (!getChildAt(i).getClass().getSimpleName().equalsIgnoreCase("button")) {
                    try {
                        throw new Exception("Only Buttons are allowed as a child of this view");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
        ...
        private void setUpButton(final Button button, final int position){
            ...
            if(position == 0){
                ...
                ((LinearLayout.LayoutParams) button.getLayoutParams()).setMargins(0, 0, mStrokeWidth*4, 0);
